...
export async function GET(_req: NextRequest, { params }: Ctx) {
  ...
  return NextResponse.json({ ok: true, insights: listInsights(id) });
}

// Post a conclusion onto the results page (the agent's analysis, or a human note).
export async function POST(req: NextRequest, { params }: Ctx) {
  const { id } = await params;
  if (!getForm(id)) return NextResponse.json({ ok: false, error: { code: "not_found", message: "No such form" } }, { status: 404 });
  ...
  const insight = addInsight(id, { text, by });
